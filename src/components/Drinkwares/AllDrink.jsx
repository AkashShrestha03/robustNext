...
const AllDrink = () => {
  ...
  const [categoryName, setCategoryName] = useState("Drinkware");
    ...
    const getProduct = async () => {
      try {
        const res = await axios.get(
          `https://spice-19.onrender.com/api/product/Category/Product/List?CategoryID=66e9507ce4a0682d9adf698a`
        );
  ...
  const fetchSortedProducts = async (order) => {
    try {
      const res = await axios.get(
        `https://spice-19.onrender.com/api/product/Sort/Product?price=${order}&CategoryID=66e9507ce4a0682d9adf698a`
      );
