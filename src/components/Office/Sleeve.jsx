...
const Sleeve = () => {
  ...
  const [subcategoryName, setSubcategoryName] = useState("Sleeve Bag");
    ...
    const getProduct = async () => {
      try {
        const res = await axios.get(
          `https://spice-19.onrender.com/api/product/Sub/Category/Product/List?SubCategoryID=673f0a13ea6d25bf45ae655f`
        );
  ...
  const fetchSortedProducts = async (order) => {
    try {
      const res = await axios.get(
        `https://spice-19.onrender.com/api/product/Sort/Product?price=${order}&SubCategoryID=673f0a13ea6d25bf45ae655f`
      );
      setProducts(res?.data?.data);
    } catch (error) {
