...
const ThreeSet = () => {
  ...
  const [subcategoryName, setSubcategoryName] = useState("3 Pcs Combo Set");
    ...
    const getProduct = async () => {
      try {
        const res = await axios.get(
          `https://spice-19.onrender.com/api/product/Sub/Category/Product/List?SubCategoryID=673475348c27f9ea164b8333`
        );
  ...
  const fetchSortedProducts = async (order) => {
    try {
      const res = await axios.get(
        `https://spice-19.onrender.com/api/product/Sort/Product?price=${order}&SubCategoryID=673475348c27f9ea164b8333`
      );
      setProducts(res?.data?.data);
    } catch (error) {
