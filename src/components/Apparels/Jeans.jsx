...
const Jeans = () => {
    ...
    const getProduct = async () => {
      try {
        const res = await axios.get(
          `${API}/api/product/Sub/Category/Product/List?SubCategoryID=670668435ad152add907f3e7`
        );
  ...
  const fetchSortedProducts = async (order) => {
    try {
      const res = await axios.get(
        `${API}/api/product/Sort/Product?price=${order}&SubCategoryID=670668435ad152add907f3e7`
      );
      setProducts(res?.data?.data);
    } catch (error) {
